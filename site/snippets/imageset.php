...
<?php elseif($imageset->outputStyle() === 'overlay'): /* Image in overlay output mode */ ?>

	<?php if($imageset->count() > 1): /* image set with multiple ratios */ ?>

		<span>A OVERLAY IMAGE(SET) WITH MULTIPLE RATIOS IS NOT SUPPORTED!</span>

	<?php else: /* image set with single ratio */ ?>

		<?php if($imageset->option('lazyload')): ?>

				<?php /* <span class="overlay-image__imageset" style="<?= (($imageset->option('placeholder') == 'color') ? 'background-color: ' . $imageset->color() . '; ' : '') . 'height: calc(' . utils::formatFloat(1 / $imageset->ratio() * 100, 10) . 'vw - 1.2rem); max-width: calc(' . utils::formatFloat($imageset->ratio() / 1 * 100, 10) . 'vh - 1.2rem);'; ?>"> */ ?>
				<span class="overlay-image__imageset" style="<?= (($imageset->option('placeholder') == 'color') ? 'background-color: ' . $imageset->color() . '; ' : '') . 'height: ' . utils::formatFloat(1 / $imageset->ratio() * 97, 10) . 'vw; max-width: ' . utils::formatFloat($imageset->ratio() / 1 * 97, 10) . 'vh;'; ?>">

					<img <?= $imageset->srcAttributes() ?>
							<?= $imageset->sizesAttributes() ?>
							<?= $imageset->altAttribute() ?>
							<?php /* class="<?= $imageset->elementClass() ?>" */ ?>
							class="overlay-image__element lazyautosizes lazyload"
							<?= $imageset->trailingSlash() ?>>

					<?php /* NO SCRIPT TAG IS NOT NECESSARY, BECAUSE OVERLAY CAN ONLY BE OPENED WITH JS! */ ?>
