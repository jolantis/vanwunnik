	...
	if($pagination) {
		...
		if($filter_value) {
			...
			if($pagination_filtered) {
				$page_items = $page_items->filterBy($filter_key, '==', tagunslug($filter_value), ',')->sortBy('year', 'asc')->paginate(c::get('pagination.' . $page->intendedTemplate(), 10), array('page' => $page_num));
			}
			# Else...
			else {
				if($page_num and $page_num != 1) go(site()->errorPage()->url(), 404);

				$page_items = $page_items->filterBy($filter_key, '==', tagunslug($filter_value), ',')->sortBy('year', 'asc');
			}
